...
class Decoder(nn.Module):
    def __init__(self,  output_3dmm_dim = 58, output_emotion_dim = 25, feature_dim = 128, device = 'cpu', 
        ...
        self.listener_reaction_3dmm_map_layer =nn.Sequential(
            nn.Linear(output_emotion_dim + feature_dim, feature_dim),
            nn.Tanh(),
            nn.Linear(feature_dim, output_3dmm_dim),
        )
        self.listener_reaction_emotion_map_layer = nn.Linear(feature_dim, output_emotion_dim)
    ...
    def forward(self, encoded_feature):
        ...
        listener_3dmm_out = self.listener_reaction_3dmm_map_layer(torch.cat((listener_emotion_out, listener_reaction), dim=-1))
...
class TransformerVAEFinal(nn.Module):
    def __init__(self, img_size=224, audio_dim = 78, output_3dmm_dim = 58, output_emotion_dim = 25, feature_dim = 128, seq_len=751, max_seq_len=751, online = False, window_size = 8, use_hubert=False, device = 'cuda'):
        super(TransformerVAEFinal, self).__init__()
