...
def train(args, model, train_loader, optimizer, criterion):
    ...
    for batch_idx, (speaker_video_clip, _, speaker_audio_clip, _, _, _, _, listener_emotion, listener_emotion_neg, listener_3dmm, _) in enumerate(tqdm(train_loader)):
        # print ("Batch: ", batch_idx)
        if torch.cuda.is_available():
            speaker_video_clip, speaker_audio_clip,  listener_emotion, listener_3dmm, listener_emotion_neg = \
                speaker_video_clip.cuda(), speaker_audio_clip.cuda(),  listener_emotion.cuda(), listener_3dmm.cuda(), listener_emotion_neg.cuda()
        # print ("Loaded to GPU.")
        listener_3dmm_out, listener_emotion_out, \
            distribution, \
                spk_encoded, listener_emt_pos, listener_emt_neg = model(speaker_video_clip, speaker_audio_clip, \
                                                                        (listener_emotion, listener_emotion_neg))
        
        loss, rec_loss, kld_loss, cont_loss = criterion(listener_emotion, listener_3dmm, 
...
def main(args):
    ...
    # train dataloader
    train_loader = get_dataloader(args, "../data/train_neg.csv", load_audio=True, load_video_s=True, load_emotion_l=True, load_3dmm_l=True, use_raw_audio=args.use_hubert, mode='train')
    # val dataloader
